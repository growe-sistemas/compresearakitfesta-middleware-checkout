...
| este repositório | middleware Express fora do VTEX IO | Hoje: **porte fiel** das 16 rotas. Amanhã: a API v2 descrita aqui. |
...
3. [Diagnóstico do legado](03-diagnostico-legado.md) — o que está inconsistente,
   quebrado ou perigoso na versão VTEX IO (e o que já foi corrigido no porte).
4. [Contratos da API v2](04-contratos-v2.md) — **as requisições melhoradas**:
   endpoint a endpoint, request e response.
5. [Plano de migração](05-plano-de-migracao.md) — ordem de execução,
   convivência entre v1 e v2, e o que vira skill.
...
A v2 inverte isso: o middleware passa a **devolver a decisão pronta**, e o front
só reage.
